...
static int internal_sheens_constraints2javascript(icLinkedListIterator* iterator, char* js, icLinkedList* rpn, bool *timeFuncsAdded)
{
    ...
    while (linkedListIteratorHasNext(iterator)) {
        ...
        if (constraint) {
            ...
            if (linkedListCount(constraint->system_constraints) > 0) {
                icLinkedListIterator* sysiter = linkedListIteratorCreate(constraint->system_constraints);

                while (linkedListIteratorHasNext(sysiter)) {
                    icrule_constraint_system_t* value = linkedListIteratorGetNext(sysiter);

                    if (value) {
                        const char* allowed_check;

                        /* On the first pass through the JS generation we only want
                         * the Boolean from the first check. This prevents us from
                         * tainting the overall result with our default. We do this
                         * because we do not now what our logic will be from call-to-call.
                         */
                        // TODO: The first_allowed can probably be reduced down to just verify the pointer
                        if (first_allowed) {
                            allowed_check = sheens_allowed_key " =";
                            first_allowed = false;
                        } else {
                            /* A system cannot be in more than one status state at a time. Thus
                             * We force the allowed check to be an OR status.
                             */
                            allowed_check = allowed_or_str;
                        }

                        /* We need to make sure the system status (scenes or panel) match our
                         * constraints. However, there may be multiple so if there are
                         * we need to make sure all the other checks have also met the requirements.
                         * We perform our Bitwise AND/OR depending on the 'logic' that is in the
                         * current constraint. Thus we can chain together all of our status checks
                         * and produce one valid true/false output.
                         */
                        switch (*value) {
                            case CONSTRAINT_SCENE_HOME:
                            case CONSTRAINT_PANEL_DISARMED:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'home');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_PANEL_ALARM:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key" '] == 'alarm');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_PANEL_ARMING:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'arming');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_PANEL_ARMED:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s "
                                                         "((bindings['" sheens_systemstatus_bound_key "'] == 'away') || "
                                                         "(bindings['" sheens_systemstatus_bound_key "'] == 'stay') || "
                                                         "(bindings['" sheens_systemstatus_bound_key "'] == 'night'));\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_SCENE_AWAY:
                            case CONSTRAINT_PANEL_ARM_AWAY:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'away');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_PANEL_ARM_STAY:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'stay');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_SCENE_NIGHT:
                            case CONSTRAINT_PANEL_ARM_NIGHT:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'night');\n",
                                                         allowed_check);
                                break;
                            case CONSTRAINT_SCENE_VACATION:
                                bytes_written += sprintf(js + bytes_written,
                                                         "  %s (bindings['" sheens_systemstatus_bound_key "'] == 'vacation');\n",
                                                         allowed_check);
                                break;
                        }
                    }
                ...
                linkedListIteratorDestroy(sysiter);
            }
