...
static ArmDisarmNotification requestArm(PanelStatus desiredState,
                                        const char *accessCode,
                                        RequestSource source,
                                        ArmDisarmNotification notif);

static ArmDisarmNotification requestDisarm(const char *accessCode, RequestSource source);

static bool requestPanic(PanelStatus desiredState, RequestSource source);

static PanelStatus getPanelStatusForArm(systemPanelStatus *securityPanelStatus);

static PanelStatus convertAlarmReasonForPanelStatus(alarmReasonType alarmReason, alarmPanicType panicReason);
...
armSourceType convertRequestSourceToArmSource(RequestSource source)
{
    armSourceType armSource = ARM_SOURCE_CPE_KEYPAD;
    switch (source)
    {
        case REQUEST_SOURCE_WIRELESS_KEYFOB:
            armSource = ARM_SOURCE_KEYFOB;
            break;
        case REQUEST_SOURCE_WIRELESS_KEYPAD:
            armSource = ARM_SOURCE_WIRELESS_KEYPAD;
            break;
        case REQUEST_SOURCE_TAKEOVER_KEYPAD:
            armSource = ARM_SOURCE_TAKEOVER_KEYPAD;
            break;
        default:
            icLogWarn(LOG_TAG,
                      "%s: Request source [%d] not recognized, defaulting to %s",
                      __FUNCTION__,
                      source,
                      armSourceTypeLabels[armSource]);
            break;
    }
...
armModeType convertPanelStatusToArmMode(PanelStatus desiredState)
{
    armModeType armMode = ARM_METHOD_NONE;
    switch (desiredState)
    {
        case PANEL_STATUS_ARMED_STAY:
            armMode = ARM_METHOD_STAY;
            break;
        case PANEL_STATUS_ARMED_AWAY:
            armMode = ARM_METHOD_AWAY;
            break;
        case PANEL_STATUS_ARMED_NIGHT:
            armMode = ARM_METHOD_NIGHT;
            break;
        default:
            icLogWarn(LOG_TAG,
                      "%s: Panel status [%d] not recognized, defaulting to %s",
                      __FUNCTION__,
                      desiredState,
                      armModeTypeLabels[armMode]);
            break;
    }
...
ArmDisarmNotification deviceServiceRequestPanelStatusChange(const PanelStatus desiredState,
                                                            const char *accessCode,
                                                            const RequestSource source)
{
    icLogDebug(LOG_TAG, "%s", __FUNCTION__);

    ArmDisarmNotification notif = ARM_NOTIF_INVALID;
    switch (desiredState)
    {
        case PANEL_STATUS_DISARMED:
            notif = requestDisarm(accessCode, source);
            break;
        case PANEL_STATUS_ARMED_NIGHT:
            notif = requestArm(desiredState, accessCode, source, ARM_NOTIF_ARMED_NIGHT);
            break;
        case PANEL_STATUS_ARMED_AWAY:
            notif = requestArm(desiredState, accessCode, source, ARM_NOTIF_ARMED_ALL);
            break;
        case PANEL_STATUS_ARMED_STAY:
            notif = requestArm(desiredState, accessCode, source, ARM_NOTIF_ARMED_HOME);
            break;
        case PANEL_STATUS_PANIC_MEDICAL:
        case PANEL_STATUS_PANIC_POLICE:
        case PANEL_STATUS_PANIC_FIRE:
            notif = requestPanic(desiredState, source) ? ARM_NOTIF_ARMED_ALL : ARM_NOTIF_NOT_READY;
            break;
        default:
            icLogError(LOG_TAG, "Unable to arm or disarm: requested panel status %d not supported", desiredState);
            break;
    }
...
bool deviceServiceRequestBypassZoneToggle(const uint32_t displayIndex, const char *code, const RequestSource source)
{
    icLogDebug(LOG_TAG, "%s: toggling bypass zone at index %"PRIu8, __FUNCTION__, displayIndex);

    bypassZoneToggleRequest *request = create_bypassZoneToggleRequest();

    if (code != NULL)
    {
        request->userCode = strdup(code);
    }

    request->bypassSource = convertRequestSourceToArmSource(source);
    request->displayIndex = displayIndex;

    bool worked = false;
    securityService_request_BYPASS_ZONE_TOGGLE(request, &worked);

    destroy_bypassZoneToggleRequest(request);

    return worked;
}

static ArmDisarmNotification requestDisarm(const char *accessCode, const RequestSource source)
{
    icLogDebug(LOG_TAG, "%s", __FUNCTION__);

    AUTO_CLEAN(pojo_destroy__auto) disarmRequest *req = create_disarmRequest();
    AUTO_CLEAN(pojo_destroy__auto) disarmResult *result = create_disarmResult();
    IPCCode rc = 0;
    ArmDisarmNotification notif = ARM_NOTIF_INVALID;


    if (source != REQUEST_SOURCE_WIRELESS_KEYFOB)
    {
        if (accessCode != NULL)
        {
            req->disarmCode = strdup(accessCode);
        }
        else
        {
            icLogError(LOG_TAG, "Unable to disarm: accessCode cannot be NULL");
            return ARM_NOTIF_BAD_ACCESS_CODE;
        }
    }

    req->armSource = convertRequestSourceToArmSource(source);

    rc = securityService_request_DISARM_SYSTEM(req, result);
    if (rc == IPC_SUCCESS)
    {
        switch (result->result)
        {
            case SYSTEM_DISARM_SUCCESS:
                notif = ARM_NOTIF_DISARMED;
                break;
            case SYSTEM_DISARM_FAIL_USERCODE:
                notif = ARM_NOTIF_BAD_ACCESS_CODE;
                break;
            case SYSTEM_DISARM_ALREADY_DISARMED:
                notif = ARM_NOTIF_ALREADY_DISARMED;
                break;
            default:
                icLogError(LOG_TAG, "Unable to disarm system: security service responded with %s",
                           disarmResultTypeLabels[result->result]);
                break;
        }
    }
    else
    {
        icLogError(LOG_TAG, "Unable to request system disarm: Unsuccessful IPC result: %s", IPCCodeLabels[rc]);
    }

    return notif;
}

static ArmDisarmNotification requestArm(const PanelStatus desiredState,
                                        const char *accessCode,
                                        const RequestSource source,
                                        const ArmDisarmNotification successNotif)
{
    icLogDebug(LOG_TAG, "%s", __FUNCTION__);

    ArmDisarmNotification retVal = ARM_NOTIF_INVALID;
    AUTO_CLEAN(pojo_destroy__auto) armRequest *req = create_armRequest();
    AUTO_CLEAN(pojo_destroy__auto) armResult *result = create_armResult();
    ...
    if (source != REQUEST_SOURCE_WIRELESS_KEYFOB)
    {
        if (accessCode != NULL)
        {
            req->armCode = strdup(accessCode);
        }
        else if (source != REQUEST_SOURCE_TAKEOVER_KEYPAD)
        {
            icLogError(LOG_TAG, "Unable to arm: accessCode cannot be NULL");
            return ARM_NOTIF_BAD_ACCESS_CODE;
        }
    }

    req->armMode = convertPanelStatusToArmMode(desiredState);
    req->armSource = convertRequestSourceToArmSource(source);
    rc = securityService_request_ARM_SYSTEM(req, result);
    ...
    if (rc == IPC_SUCCESS)
    {
        switch (result->result)
        {
            ...
            case SYSTEM_ARM_FAIL_USERCODE:
                retVal = ARM_NOTIF_BAD_ACCESS_CODE;
                break;

            case SYSTEM_ARM_FAIL_ZONE:
                retVal = ARM_NOTIF_NOT_READY;
                break;

            case SYSTEM_ARM_FAIL_TROUBLE:
                retVal = ARM_NOTIF_TROUBLE;
                break;

            case SYSTEM_ARM_ALREADY_ARMED:
                retVal = ARM_NOTIF_ALREADY_ARMED;
                break;

            default:
                retVal = ARM_NOTIF_NOT_READY;
                icLogError(LOG_TAG, "Unable to arm system: %s", armResultTypeLabels[result->result]);
                break;
        }
    }
    else
    {
        icLogError(LOG_TAG, "Unable to request system arm: Unsuccessful IPC result: %s", IPCCodeLabels[rc]);
    }

    return retVal;
}

static bool requestPanic(const PanelStatus desiredState, const RequestSource source)
{
    bool didPanic = false;
    alarmPanicType which = PANIC_ALARM_TYPE_NONE;
    switch (desiredState)
    {
        case PANEL_STATUS_PANIC_FIRE:
            which = PANIC_ALARM_TYPE_FIRE;
            break;
        case PANEL_STATUS_PANIC_MEDICAL:
            which = PANIC_ALARM_TYPE_MEDICAL;
            break;
        case PANEL_STATUS_PANIC_POLICE:
            which = PANIC_ALARM_TYPE_POLICE;
            break;
        default:
            icLogWarn(LOG_TAG, "%s: Unsupported panel status for panic: %s", __FUNCTION__,
                      PanelStatusLabels[desiredState]);
            break;
    }

    if (which != PANIC_ALARM_TYPE_NONE)
    {
        AUTO_CLEAN(pojo_destroy__auto) panicRequest *request = create_panicRequest();
        request->panicType = which;
        request->armSource = convertRequestSourceToArmSource(source);
        securityService_request_START_PANIC_ALARM(request, &didPanic);
    }
...
static PanelStatus getPanelStatusForArm(systemPanelStatus *securityPanelStatus)
{
    PanelStatus panelStatus = PANEL_STATUS_INVALID;
    bool isArming = securityPanelStatus->alarmStatus == ALARM_STATUS_ARMING;
    switch (securityPanelStatus->armMode)
    {
        case ARM_METHOD_STAY:
            panelStatus = isArming ? PANEL_STATUS_ARMING_STAY : PANEL_STATUS_ARMED_STAY;
            break;
        case ARM_METHOD_AWAY:
            panelStatus = isArming ? PANEL_STATUS_ARMING_AWAY : PANEL_STATUS_ARMED_AWAY;
            break;
        case ARM_METHOD_NIGHT:
            panelStatus = isArming ? PANEL_STATUS_ARMING_NIGHT : PANEL_STATUS_ARMED_NIGHT;
            break;
        case ARM_METHOD_NONE:
            panelStatus = PANEL_STATUS_DISARMED;
            break;
        default:
            icLogWarn(LOG_TAG, "Unsupported system panel status %s", armModeTypeLabels[securityPanelStatus->armMode]);
            break;
    }
...
static PanelStatus convertAlarmReasonForPanelStatus(alarmReasonType alarmReason, alarmPanicType panicReason)
{
    ...
    switch (panicReason)
    {
        case PANIC_ALARM_TYPE_NONE:
            isPanic = false;
            break;
        case PANIC_ALARM_TYPE_FIRE:
            panelStatus = PANEL_STATUS_PANIC_FIRE;
            break;
        case PANIC_ALARM_TYPE_MEDICAL:
            panelStatus = PANEL_STATUS_PANIC_MEDICAL;
            break;
        case PANIC_ALARM_TYPE_POLICE:
        case PANIC_ALARM_TYPE_POLICE_SILENT:
        case PANIC_ALARM_TYPE_SILENT_BURGLARY:
            panelStatus = PANEL_STATUS_PANIC_POLICE;
            break;
        default:
            isPanic = false;
            icLogWarn(LOG_TAG, "%s: Panic reason %s not supported", __FUNCTION__, alarmPanicTypeLabels[panicReason]);
            break;
    }
    ...
    if (!isPanic)
    {
        switch (alarmReason)
        {
            case ALARM_REASON_NONE:
                panelStatus = PANEL_STATUS_ALARM_NONE;
                break;
            case ALARM_REASON_FIRE:
                panelStatus = PANEL_STATUS_ALARM_FIRE;
                break;
            case ALARM_REASON_MEDICAL:
                panelStatus = PANEL_STATUS_ALARM_MEDICAL;
                break;
            case ALARM_REASON_CO:
                panelStatus = PANEL_STATUS_ALARM_CO;
                break;
            case ALARM_REASON_DURESS_SILENT:
                panelStatus = PANEL_STATUS_PANIC_POLICE;
                break;
            case ALARM_REASON_BURGLARY_SILENT:
            case ALARM_REASON_BURGLARY:
            case ALARM_REASON_SYSTEM_TAMPER:
            case ALARM_REASON_ZONE_COMM_FAIL:
            case ALARM_REASON_ZONE_TAMPER:
                panelStatus = PANEL_STATUS_ALARM_BURG;
                break;
            case ALARM_REASON_ENVIRONMENTAL:
            case ALARM_REASON_EXIT_ERROR:
            case ALARM_REASON_AUDIBLE:
                panelStatus = PANEL_STATUS_ALARM_AUDIBLE;
                break;
            default:
                icLogWarn(LOG_TAG, "Unsupported alarm status: %s", alarmReasonTypeLabels[alarmReason]);
                break;
        }
    }
...
SecurityState *deviceServiceGetSecurityState(void)
{
    ...
    AUTO_CLEAN(pojo_destroy__auto) systemPanelStatus *status = create_systemPanelStatus();
    alarmReasonType alarmReason = ALARM_REASON_NONE;
    alarmPanicType panicReason = PANIC_ALARM_TYPE_NONE;
    SecurityIndication indication = SECURITY_INDICATION_NONE;
    uint32_t exitDelay = 0;
    IPCCode rc;

    rc = securityService_request_GET_SYSTEM_PANEL_STATUS(status);
    if (rc != IPC_SUCCESS)
    {
        icLogError(LOG_TAG, "%s: unable to read system panel status: %s", __FUNCTION__, IPCCodeLabels[rc]);
    }
    exitDelay = deviceServiceGetSecurityExitDelay();
    if (status->alarmStatus == ALARM_STATUS_ALARM)
    {
        AUTO_CLEAN(pojo_destroy__auto) currentAlarmStatus *alarmInfo = create_currentAlarmStatus();
        rc = securityService_request_GET_CURRENT_ALARM_STATUS(alarmInfo);
        if (rc != IPC_SUCCESS)
        {
            icLogError(LOG_TAG, "%s: unable to read current alarm status: %s", __FUNCTION__, IPCCodeLabels[rc]);
        }
        else
        {
            alarmReason = alarmInfo->alarm->alarmReason;
            panicReason = alarmInfo->panicType;
        }

        /* We will not know if we are in a silent alarm, so all alarms are considered noisy. */
        indication = SECURITY_INDICATION_BOTH;
    }

    PanelStatus panelStatus = deviceServiceConvertSystemPanelStatus(status, alarmReason, panicReason);
    SecurityState *state = securityStateCreate(panelStatus, exitDelay, indication, status->bypassActive);
...
PanelStatus deviceServiceConvertSystemPanelStatus(systemPanelStatus *sysPanelStatus, alarmReasonType alarmReason,
                                                  alarmPanicType panicReason)
{
    PanelStatus panelStatus = PANEL_STATUS_INVALID;

    switch (sysPanelStatus->alarmStatus)
    {
        case ALARM_STATUS_ALARM:
            panelStatus = convertAlarmReasonForPanelStatus(alarmReason, panicReason);
            break;
        case ALARM_STATUS_NOTREADY:
            panelStatus = PANEL_STATUS_UNREADY;
            break;
        case ALARM_STATUS_READY:
            panelStatus = PANEL_STATUS_DISARMED;
            break;
        case ALARM_STATUS_ENTRY_DELAY:
            panelStatus = PANEL_STATUS_ENTRY_DELAY;
            break;
        case ALARM_STATUS_ARMED:
        case ALARM_STATUS_ARMING:
            panelStatus = getPanelStatusForArm(sysPanelStatus);
            break;
        case ALARM_STATUS_UNCHANGED:
            break;
        default:
            icLogWarn(LOG_TAG, "Unsupported system alarm status %s",
                      alarmStatusTypeLabels[sysPanelStatus->alarmStatus]);
            break;
    }
...
{
    SecurityIndication indication = SECURITY_INDICATION_NONE;
    switch (securityServiceIndication)
    {
        case INDICATION_NONE:
            indication = SECURITY_INDICATION_NONE;
            break;
        case INDICATION_AUDIBLE:
            indication = SECURITY_INDICATION_AUDIBLE;
            break;
        case INDICATION_BOTH:
            indication = SECURITY_INDICATION_BOTH;
            break;
        case INDICATION_VISUAL:
            indication = SECURITY_INDICATION_VISUAL;
            break;
        default:
            icLogWarn(LOG_TAG,
                      "%s: Unsupported system indication type %s, defaulting to [%d]",
                      __FUNCTION__,
                      indicationTypeLabels[securityServiceIndication],
                      indication);
            break;
    }
...
{
    uint32_t exitDelay;
    IPCCode rc = securityService_request_GET_EXIT_DELAY(&exitDelay);
    if (rc != IPC_SUCCESS)
    {
        icLogError(LOG_TAG, "Getting exit delay from security service failed: %s", IPCCodeLabels[rc]);
        exitDelay = 0;
    }
...
bool deviceServiceFirmwareUpgradesAllowed()
{
    ...
    //we can upgrade when disarmed or unready (unready means zone faulted or tampered)
    if (state->panelStatus == PANEL_STATUS_DISARMED || state->panelStatus == PANEL_STATUS_UNREADY)
    {
        icLogDebug(LOG_TAG,
                   "%s: allowing firmware upgrades since system panel is %s",
                   __FUNCTION__,
    ...
    {
        icLogDebug(LOG_TAG,
                   "%s: NOT allowing firmware upgrades since system panel status is %s",
                   __FUNCTION__,
                   PanelStatusLabels[state->panelStatus]);
    }
