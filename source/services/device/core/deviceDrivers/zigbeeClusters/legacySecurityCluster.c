...
{
    bool ok = true;
    memset(panelState, 0, sizeof(uCPanelStateMessage));

    switch (state->panelStatus)
    {
        case PANEL_STATUS_DISARMED:
            panelState->alarmEvent = PANEL_STATE_DISARMED;
            break;

        case PANEL_STATUS_UNREADY:
            panelState->alarmEvent = PANEL_STATE_NOT_READY;
            break;

        case PANEL_STATUS_ARMING_STAY:
            panelState->alarmEvent = PANEL_STATE_ARMING;
            panelState->armType = ARM_TYPE_STAY;
            break;

        case PANEL_STATUS_ARMING_AWAY:
            panelState->alarmEvent = PANEL_STATE_ARMING;
            panelState->armType = ARM_TYPE_AWAY;
            break;

        case PANEL_STATUS_ARMING_NIGHT:
            panelState->alarmEvent = PANEL_STATE_ARMING;
            panelState->armType = ARM_TYPE_NIGHT;
            break;

        case PANEL_STATUS_ARMED_STAY:
            panelState->alarmEvent = PANEL_STATE_ARMED;
            panelState->armType = ARM_TYPE_STAY;
            break;
        ...
        case PANEL_STATUS_ENTRY_DELAY:
            panelState->alarmEvent = PANEL_STATE_ENTRY_DELAY_REMAINING;
            panelState->delayRemaining = state->timeLeft;
            break;

        case PANEL_STATUS_ENTRY_DELAY_ONESHOT:
            panelState->alarmEvent = PANEL_STATE_ENTRY_DELAY;
            panelState->delayRemaining = state->timeLeft;
            break;

        case PANEL_STATUS_EXIT_DELAY:
            panelState->alarmEvent = PANEL_STATE_EXIT_DELAY_REMAINING;
            panelState->delayRemaining = state->timeLeft;

            /*
             * Arm type away is used by PIMs to figure out when to change cadence.
             * Other arm types have a non-audible indication.
             */
            if (state->indication == SECURITY_INDICATION_AUDIBLE || state->indication == SECURITY_INDICATION_BOTH)
            {
                panelState->armType = ARM_TYPE_AWAY;
            }
            break;
        ...
        case PANEL_STATUS_ALARM_MEDICAL:
            panelState->alarmEvent = PANEL_STATE_ALARM;
            switch (state->panelStatus)
            {
                case PANEL_STATUS_ALARM_BURG:
                    panelState->alarmType = ALARM_TYPE_BURG;
                    break;

                case PANEL_STATUS_PANIC_POLICE:
                    panelState->alarmType = ALARM_TYPE_POLICE_PANIC;
                    break;

                case PANEL_STATUS_PANIC_MEDICAL:
                    panelState->alarmType = ALARM_TYPE_MEDICAL_PANIC;
                    break;

                case PANEL_STATUS_ALARM_MEDICAL:
                    panelState->alarmType = ALARM_TYPE_MEDICAL;
                    break;

                case PANEL_STATUS_PANIC_FIRE:
                    panelState->alarmType = ALARM_TYPE_FIRE_PANIC;
                    break;

                case PANEL_STATUS_ALARM_FIRE:
                    panelState->alarmType = ALARM_TYPE_FIRE;
                    break;

                case PANEL_STATUS_ALARM_AUDIBLE:
                case PANEL_STATUS_ALARM_POLICE:
                case PANEL_STATUS_ALARM_CO:
                    if (state->indication == SECURITY_INDICATION_BOTH ||
                        state->indication == SECURITY_INDICATION_AUDIBLE)
                    {
                        panelState->alarmType = ALARM_TYPE_BURG;
                    }
                    break;
        ...
        default:
            icLogWarn(LOG_TAG, "%s: ignoring unsupported panel status %s", __FUNCTION__,
                      PanelStatusLabels[state->panelStatus]);
            ok = false;
            break;
    }
    ...
    if (ok)
    {
        panelState->silent = state->indication == SECURITY_INDICATION_NONE ||
                             state->indication == SECURITY_INDICATION_VISUAL;
    }
                                 ...
                                 bool forceAllowed)
{
    ...
    if (forceAllowed == false && deviceServiceFirmwareUpgradesAllowed() == false)
    {
