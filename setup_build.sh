...
# 'build-model'.  uses the buildTools/setup_env.sh
# utility to get things going.  unlike setup_env.sh, this
# can be executed and doesn't require a TARGET_PLATFORM
# variable to be set.
#
#-----------------------------------------------------
...
showUsage()
{
    ...
    bash ${ZILKER_SDK_TOP}/buildTools/setup_env.sh -l;
    echo "";
}
...
runBuildThirdParty()
{
    ...
    echo "cmake -G \"Unix Makefiles\" -C ${top}/buildTools/cmake/products/${BUILD_MODEL}.cmake $buildDebugFlag $zithTestOptions ${top}/3rdParty"
    cmake -G "Unix Makefiles" -C ${top}/buildTools/cmake/products/${BUILD_MODEL}.cmake $buildDebugFlag $zithTestOptions ${top}/3rdParty
...
# first, make sure we are in the ZILKER_SDK_TOP directory
#
c=`pwd`
if [ "$c" != "$ZILKER_SDK_TOP" ]; then
    echo "Must run $0 from \$ZILKER_SDK_TOP.  Unable to continue";
    exit 1;
fi
...
# reload the environment for this platform, which will set CROSS_OUTPUT properly
#
cd $ZILKER_SDK_TOP
. ./buildTools/setup_env.sh reset
. ./buildTools/setup_env.sh ${platformArg}
...
if [ $doBuildThirdParty -eq 1 ]; then
    ...
    # special case of Android
    fixDroid=0;
    if [[ "$BUILD_TCHAIN" = "android_ndk"* ]]; then

        # need to unset target toolchain (ndk14) and reload the
        # environment using the tchain instead of the model.  For ex:
        #  tca203 --> android_ndk14
        #
        chain=${BUILD_TCHAIN};
        model=${BUILD_MODEL};
        fixDroid=1;
        . ${ZILKER_SDK_TOP}/buildTools/setup_env.sh reset;
        . ${ZILKER_SDK_TOP}/buildTools/setup_env.sh ${chain} ${model};

        # We destroy our BUILD_MODEL when we do this
        # however, we need the build model so that we load the
        # correct product for 3rdparty
        export BUILD_MODEL=${platformArg}
    fi

    runBuildThirdParty;

    # put back if we did the shift for Android
    if [ $fixDroid -eq 1 ]; then
        . ${ZILKER_SDK_TOP}/buildTools/setup_env.sh reset;
        . ${ZILKER_SDK_TOP}/buildTools/setup_env.sh ${platformArg};
    fi
fi
